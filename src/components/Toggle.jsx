...
export const Toggle = ({ enabled, onChange }) => {
  return (
    <button
      ...
      className={`relative inline-flex h-5 w-11 items-center transition-all focus:outline-none border ${
    ...
    >
      <span
        className={`inline-block h-3 w-3 transform bg-white transition-transform ${
          enabled ? 'translate-x-7 bg-accent shadow-[0_0_5px_#fff]' : 'translate-x-1 bg-gray-600'
        }`}
      />
    </button>
  );
